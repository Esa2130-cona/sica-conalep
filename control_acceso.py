...
# ================= MÓDULO: PUERTA DE ENTRADA (DISEÑO PREMIUM) =================
elif menu == "Puerta de Entrada":

    # 1. FUNCIÓN DE CONTEO (Llamada directa para evitar delays)
    def obtener_conteo_hoy():
        try:
            fecha_hoy = datetime.now(zona).strftime("%Y-%m-%d")
            res = supabase.table("entradas").select("id", count="exact").eq("fecha", fecha_hoy).execute()
            return res.count if res.count else 0
        except: return 0

    # --- INDICADOR DE ALUMNOS (DISEÑO GRANDE EN EL MEDIO/ARRIBA) ---
    presentes = obtener_conteo_hoy()
    st.markdown(f"""
        <div style='text-align: center; background: #1e1e1e; padding: 20px; border-radius: 15px; border: 1px solid #333; margin-bottom: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.5);'>
            <p style='color: #888; font-size: 16px; margin: 0; text-transform: uppercase; letter-spacing: 2px;'>Alumnos en Plantel Hoy</p>
            <h1 style='color: #00e676; font-size: 80px; margin: 10px 0; font-family: sans-serif;'>{presentes}</h1>
            <div style='width: 50px; height: 3px; background: #00e676; margin: 0 auto; border-radius: 5px;'></div>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
        <div class='scan-card'>
            ...
            <div class='scan-title'>CONTROL DE ACCESO</div>
        </div>
    """, unsafe_allow_html=True)
    ...
    # --- INTERFAZ DE ESCANEO (INSTANTÁNEA) ---
    st.markdown("<br>", unsafe_allow_html=True)
    
    _, col_input, _ = st.columns([1, 2, 1])
    with col_input:
        # Quitamos placeholder y status para que no se bloquee
        ...
        try:
            ...
            if not al_res.data:
                ...
                if al.get("estatus") is False:
                    st.session_state.resultado = {"tipo": "bloqueado", "nombre": al.get("nombre"), "mensaje": "BLOQUEADO"}
                else:
            ...
            # Limpiar input y refrescar pantalla para mostrar resultado
    ...
    if st.session_state.resultado:
        ...
        if res["tipo"] == "ok":
            ...
            st.markdown(f"""
                <div style='text-align:center; background:rgba(30,132,73,0.2); padding:40px; border-radius:20px; border:3px solid #00e676;'>
                    <div style='font-size:25px; color:#00e676;'>✅ ACCESO PERMITIDO</div>
                    <div style='font-size:50px; font-weight:900; color:white; line-height:1;'>{res['nombre']}</div>
                    <div style='font-size:30px; color:#aaa; margin-top:10px;'>GRUPO: {res['grupo']}</div>
                </div>
            """, unsafe_allow_html=True)
            if res.get("aviso"):
                av = res["aviso"]; color_av = "#ff1744" if av["prioridad"] == "ALTA" else "#ffeb3b"
                st.markdown(f"<div style='margin-top:15px; padding:15px; background:#222; border-left:10px solid {color_av}; border-radius:10px;'><h3 style='color:{color_av};'>⚠️ AVISO {av['prioridad']}</h3><p style='font-size:20px; color:white;'>{av['mensaje']}</p></div>", unsafe_allow_html=True)
        
        elif res["tipo"] == "warning":
            st.markdown("<div class='flash-overlay flash-warn'></div>", unsafe_allow_html=True)
            st.markdown(f"<div style='text-align:center; background:rgba(255,152,0,0.1); padding:40px; border-radius:20px; border:3px solid #ff9800;'><div style='font-size:40px; color:#ff9800;'>⚠️ {res['mensaje']}</div><div style='font-size:45px; font-weight:900; color:white;'>{res['nombre']}</div></div>", unsafe_allow_html=True)
        
        elif res["tipo"] == "bloqueado":
            st.markdown("<div class='flash-overlay flash-error'></div>", unsafe_allow_html=True)
            st.markdown(f"<div style='text-align:center; background:rgba(255,23,68,0.1); padding:40px; border-radius:20px; border:3px solid #ff1744;'><div style='font-size:40px; color:#ff1744;'>⛔ {res['mensaje']}</div><div style='font-size:45px; font-weight:900; color:white;'>{res['nombre']}</div></div>", unsafe_allow_html=True)
